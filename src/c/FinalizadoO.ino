...
#define FIREBASE_HOST "sensorbio-ceac7-default-rtdb.firebaseio.com/"
#define FIREBASE_AUTH "G3kPVqMehyaBrhW05O3qLPU86psQ4Jdu2SVWkoGw"
#define WIFI_SSID "HRS"
#define WIFI_PASSWORD "sabrinaa123"
#define LED_PIN 5
#define LED_PIN2 4
#define BTN_PIN 14
#define RELAY 2
...
void loop() {
  ...
  if (sensorValue == HIGH) {
    ...
    FirebaseData firebaseData;
    if (Firebase.setString(firebaseData, "sensor_status", "ligado")) {
      Serial.println("Sensor status atualizado no Firebase.");
    } else {
      Serial.println("Erro ao atualizar o status do sensor no Firebase.");
      Serial.println(firebaseData.errorReason());
    }
  } else {
    Serial.println("Sensor desligado!!!");
    FirebaseData firebaseData;
    if (Firebase.setString(firebaseData, "sensor_status", "desligado")) {
      Serial.println("Sensor status atualizado no Firebase.");
    } else {
      Serial.println("Erro ao atualizar o status do sensor no Firebase.");
      Serial.println(firebaseData.errorReason());
  ...
  // Envia a digital e nome não definido para o firebase
...
if (Firebase.getBool(firebaseData, path) || verifLocal == 1) {
  ...
  if (Firebase.getString(firebaseData, path2)) {
    ...
    sendToFirebase(String(id), firebaseData.stringData(), "Entrou na sala");
  } else {
    String name = "none";
...
void sendToFirebase(String id, String name, String message) {
  String path = "logs/id_"+ String(id);
  String logMessage = name + " - " + message;
  Firebase.pushString(firebaseData, path, logMessage); // envia a mensagem formatada para o Firebase
}
